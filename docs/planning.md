...
\- **\*\*Validation\*\***: Zod for every schema (NodeIR, LayoutIR, VisualIR, DesignIntentProposal, Patch, benchmark manifest) — matches §12A.3 "AI must return validated JSON" and §12A.4 "JSON Schema or Zod."
...
\- \`packages/model-adapters\` + \`ai-router\`: a provider-neutral adapter interface generalizing the §12A.1 model-strategy roles to implementation escalation (not the excluded pre-compiler Design Understanding stage ruled out in §2) — Gemini (primary visual analyzer), OpenAI (complex reasoning/disagreement), Claude (secondary reasoning/code review), optional Groq. Routing table from §12A.2 as data, not scattered branching.
